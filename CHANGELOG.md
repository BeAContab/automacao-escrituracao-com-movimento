...
### Adicionado
- **Escriturar ou simular as notas de cada empresa no Multi-CNPJ (backend)** (`escrituracao_multi_robo.py`; `escrituracao_multi_cnpj.py`; `tests/test_escrituracao_multi_etapa2.py`): passo 2 da Etapa 2. Ainda não há botão na tela (vem no passo 3). O orquestrador agora tem **três modos**: *validar acessos* (Etapa 1, sem alteração), ***simular*** (preenche todos os campos exatamente como na escrituração real e **para antes de "Gravar"**; nunca grava, nunca lê nem escreve o estado de retomada) e ***escriturar de verdade***. A função "Escrituração" individual continua **intacta** (hash SHA-256 conferido).
- **Laço das notas da empresa** (`processar_notas`): mesmas regras e mesma ordem do original (prestador de Fortaleza/CE que não é MEI é ignorado; campo obrigatório ausente = incompleta; duplicata do portal não é gravada), mais: pula notas já escrituradas numa execução anterior; falha em uma nota não interrompe as demais; na simulação a nota é descartada com "Novo Documento" e, **se o portal pedir qualquer confirmação, o robô não clica em nada** e interrompe a empresa (`ModalInesperadoError`); após pausa longa (≥ 10 min) confere se a sessão do portal ainda está ativa e, se expirou, interrompe a empresa (`SessaoExpiradaError`).
- **Navegação até "Digitar Documento"** (`preparar_tela_digitar_documento`): Escrituração > Manter Escrituração, competência, Consultar, Escriturar, aba Serviços Tomados, Digitar Documento. Se a competência estiver bloqueada, a empresa é pulada como "Competência indisponível" (o original perguntava outra competência no terminal). Diferença deliberada: o botão Escriturar espera 45 s (não 120 s) para uma competência bloqueada não travar cada empresa por 2 minutos.
- **Pré-análise offline por empresa** (antes de abrir o portal): "N nota(s): X a escriturar, Y ignorada(s) (Fortaleza/CE não MEI), Z incompleta(s)".
- **Logs unificados por execução** (`RegistroNotas`): um único conjunto de arquivos **na pasta da planilha** (sem subpasta `log/`) — `log_escrituradas_sucesso.txt`, `log_prefeitura_fortaleza.txt`, `log_notas_incompletas.txt`, `log_notas_duplicadas.txt`, `log_notas_com_erro.txt` e `log_notas_simuladas.txt` — com o **CNPJ da empresa em cada linha**. Os arquivos **nunca são apagados**: cada execução acrescenta um cabeçalho (data, competência e modo). Notas simuladas ficam num arquivo próprio e nunca no log de sucesso.
- **Retomada por competência e por nota** (`escrituracao_multi_cnpj.py`): o estado (`<planilha>.escrituracao_multi_estado.json`, escrita atômica, sem login nem senha) guarda, por competência, as empresas concluídas e as notas já escrituradas. Empresa com alguma nota com erro fica **"Concluída com pendências"**, não é marcada como concluída e, na próxima execução, **só as notas com erro são refeitas**. Novos status: Simulada, Concluída com pendências, Competência indisponível e Sessão expirada.
- **O estado não é apagado ao concluir tudo** (mudança em relação à Etapa 1): uma nova execução por engano da mesma planilha/competência pula as empresas já concluídas em vez de reescriturá-las; o log informa qual arquivo apagar para refazer.
- 51 testes novos (driver falso; **nenhum toca o portal nem grava nota**; um "detonador" no `setUp` derruba qualquer teste que tente clicar em Gravar). Teste de mutação feito à mão: quebrar de propósito a simulação, a guarda de gravação ou o religamento do bloqueio faz a suíte falhar (6, 5 e 6 falhas), e o arquivo foi restaurado idêntico.
- **Não validado no portal real** (por definição: exigiria gravar): o modo "escriturar de verdade". A simulação também ainda não foi vista no portal real (dois pontos em aberto: se "Novo Documento" com a nota preenchida pede confirmação de descarte — nesse caso o robô interrompe a empresa sem clicar — e se a espera de 45 s do botão Escriturar é suficiente).
...
### Adicionado
- **Pausar, Continuar e Parar agora são registrados no log da tela e no arquivo de log** (`app.py`, `_ControleProcessamentoXml.avisar`, `pausar_processamento_xml`/`retomar_processamento_xml`/`cancelar_processamento_xml`; `gui/js/tab-esc-multi.js`, `tab-xml.js`, `tab-xml-multi.js`): em teste real, ao clicar em Pausar na Escrituração Multi-CNPJ nada aparecia no log, porque os botões só escreviam na tela (e nada no arquivo) e a pausa só vale entre empresas. Agora, para "Processar XMLs", "Processar XMLs — Multi-CNPJ" e "Escrituração — Multi-CNPJ", o log mostra "PAUSA solicitada pelo operador — <o que vai acontecer>", "RETOMADA pelo operador" e "PARADA solicitada pelo operador", sempre na tela e no arquivo, uma única vez por clique. Os avisos que só existiam no JavaScript foram removidos para não duplicar.
- **Escrituração Multi-CNPJ deixa claro quando a pausa entra em vigor** (`app.py`, `_executar_escrituracao_multi`): ao chegar na pausa, o log diz "Execução PAUSADA antes da próxima empresa (a anterior já fez logout)…"; se a pausa foi pedida durante a última empresa, avisa "A pausa solicitada não chegou a valer: a execução terminou antes de haver uma próxima empresa" (a pausa só vale entre empresas).
- 5 testes novos no nível do `app.py` (pausa no meio da execução com o log em arquivo e retomada, pausa na última empresa, parada, cliques repetidos, controles sem execução em andamento).

### Corrigido
- **Logout da Escrituração Multi-CNPJ ainda falhava com o CNPJ inexistente** (`escrituracao_multi_cnpj.py`, `AcessoIssSelenium.logout`): o diagnóstico da `2.41.1` mostrou que o login único confirmava a saída, mas o portal terminava em `home.seam` **sem o botão "Fazer login"** — ou seja, o ISS continuava logado (o clique em "Sair" do ISS fora bloqueado e o logout do login único não encerra a sessão do ISS). O logout agora é uma **máquina de estados** que, a cada volta, olha a tela e faz o passo seguinte, sem depender da ordem: confirma no login único se pedido; se o ISS ainda está logado, clica em "Sair" (fechando antes qualquer modal RichFaces, inclusive por classe `rich-modalpanel`, e desativando o aviso "sair desta página?"; se algo cobrir o botão, registra o motivo exato e força o clique por script); se o estado for desconhecido, vai direto ao logout do login único; termina quando aparece "Fazer login". Limite de 45 s no total. O diagnóstico de falha agora inclui o motivo do clique bloqueado (tipo e mensagem do erro do Selenium).
- Testes do logout reescritos com navegador simulado, incluindo o cenário do teste real (login único sai, ISS continua logado).

## [2.41.1] - 2026-09-21

### Corrigido
- **Logout da Escrituração Multi-CNPJ falhava depois de "empresa não encontrada"** (`escrituracao_multi_cnpj.py`, `AcessoIssSelenium.logout`): em teste real com um CNPJ que não existe no login, o robô pulou a empresa corretamente, mas o logout levou 32 s e não foi confirmado (o navegador foi descartado com a sessão ainda aberta no portal). A tela de seleção de empresas ficava aberta com a busca sem resultado, e o robô só fechava o modal "Dados do contribuinte". Agora, antes de sair, o robô fecha **qualquer modal RichFaces aberto** e desativa o aviso "sair desta página?" do Chrome (`window.onbeforeunload`); alertas do navegador que travem a leitura da página durante o logout são aceitos; e, se a primeira tentativa não confirmar, faz uma **segunda tentativa** direto pelo logout do login único. A espera final caiu de 30 s para 15 s por tentativa.
- **Mensagem de logout enganosa**: o log dizia "vou reabrir o navegador" mesmo quando não havia mais empresas. Agora diz "vou apenas fechar o navegador" no último caso.

### Adicionado
- **Diagnóstico da falha de logout**: o aviso "Não consegui confirmar o logout" passa a trazer o detalhe das etapas (tentativa, clique em Sair bloqueado ou não, modais fechados, se a confirmação do login único apareceu) e a página em que o robô parou (sem parâmetros de URL e sem dados sensíveis), para localizar a causa exata caso volte a acontecer. Quando o logout dá certo, o log registra "Logout confirmado.".
- 7 testes novos (logout normal, botão "Sair" bloqueado por modal, alerta do navegador aceito, falha com diagnóstico sem query string, mensagens conforme restem ou não empresas, log de sucesso), com um navegador simulado. **Ainda não validado no portal real**: repetir o teste com o CNPJ inexistente.

## [2.41.0] - 2026-09-21

### Adicionado
- **Nova função "Escrituração" no grupo Multi-CNPJ — Etapa 1: validar acessos** (novo `escrituracao_multi_cnpj.py`; `app.py`, `iniciar_escrituracao_multi_gui`; `gui/index.html`; `gui/js/tab-esc-multi.js`; `gui/js/app-shell.js`; `tests/test_escrituracao_multi_cnpj.py`): a partir da planilha gerada em "Processar XMLs — Multi-CNPJ" (com `CNPJ_TOMADOR`, `LOGIN` e `SENHA` preenchidos), o robô percorre as empresas e, para cada uma: abre o portal ISS Fortaleza, faz login com as credenciais da empresa, seleciona a inscrição pelo CNPJ (rádio CNPJ → digitar → aguardar a sugestão → selecionar; usa o atalho se a empresa já estiver listada), **confere o CNPJ** lendo "Dados do contribuinte" (se divergir, tenta trocar de inscrição uma vez e, se persistir, marca "CNPJ divergente") e faz **logout** (Sair do ISS + `#kc-logout` do login único). **Nenhuma nota é escriturada nesta etapa** (a caixa "Apenas validar acessos" vem marcada e travada); a escrituração por empresa é a Etapa 2.
  - **Empresa com problema é pulada** e a execução segue: planilha inválida (LOGIN/SENHA em branco, valores diferentes entre linhas do mesmo CNPJ ou CNPJ com menos de 14 caracteres — antes de qualquer acesso, sem abrir navegador se não houver o que acessar), login recusado (**sem nova tentativa**, para não bloquear o usuário no portal), empresa não encontrada, CNPJ divergente ou erro inesperado. O resumo final lista CNPJ e situação de cada empresa.
  - **Logout sempre**: mesmo com erro ou cancelamento o robô sai do portal; se o logout não for confirmado, descarta o navegador (sessão suja) e abre outro para a próxima empresa. Ao terminar ou ao parar, faz logout e fecha o navegador. Há Pausar/Continuar/Parar (a pausa vale entre empresas; a parada finaliza a etapa em andamento e marca as demais como "Cancelada").
  - **Retomada preparada** (usada na Etapa 2): arquivo `<planilha>.escrituracao_multi_estado.json` guarda só os CNPJs concluídos (nunca login/senha); reexecutar pula as concluídas e tenta de novo as que falharam; o arquivo some quando todas concluem. Na Etapa 1 (só validar) ele não é criado.
  - **Segurança**: senha só em memória (fora do `repr`), nunca no log, no resumo da GUI, nas mensagens de erro (mensagens passam por sanitização) nem no arquivo de retomada; o login aparece mascarado (`***.***.***-NN`). LOGIN numérico é normalizado (o Excel remove o zero à esquerda do CPF); a SENHA nunca é alterada.
  - **Log único** no arquivo `log_execucao_<data_hora>.txt`, na mesma pasta da planilha, com o CNPJ em cada linha; mutuamente exclusivo com a "Escrituração" comum (as duas abrem o portal ISS).
  - Validado com 24 testes automatizados (leitura/agrupamento da planilha, normalização, senhas divergentes sem vazar valores, orquestração com portal simulado: pular empresa com erro, logout sempre, reabrir navegador se o logout falhar, cancelamento, retomada). **Não validado contra o portal real** (exige as contas de verdade): detecção de login recusado, tempo da sugestão de CNPJ, fechamento do modal "Dados do contribuinte", modal de confirmação "Sim" da troca de inscrição, pop-ups de aviso do portal e o caminho alternativo de logout.
