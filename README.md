...
* **Suporte a Pasta com Subpastas:** Detecção automática da estrutura da pasta selecionada — se houver XMLs diretamente nela, gera uma única planilha (comportamento padrão); se a pasta contiver subpastas (ex.: uma por cliente/lote), gera uma planilha independente dentro de cada subpasta, sem misturar notas de lotes diferentes num único arquivo.
...
* **Preenchimento Guiado:** Alimentação automatizada de todos os campos da escrituração (CNPJ, número da nota, data de emissão, código do serviço, descrição, alíquota, valores e retenção de ISS), com verificação e log do estado final do checkbox de ISS Retido para cada nota. Os campos de texto mais longos são preenchidos instantaneamente, com verificação automática e nova tentativa por digitação caractere a caractere caso o valor não seja aceito de primeira.
...
* **Resolução Automática de Confirmações do Portal:** O robô reconhece e resolve sozinho os principais bloqueios que o portal pode exibir ao gravar uma nota — nota fiscal duplicada (não confirma a gravação e registra em `log_notas_duplicadas.txt`), avisos informativos como "Prestador não inscrito no CPOM" (confirma e tenta gravar novamente), e o erro "CNAE não incide Imposto Sobre Serviço" (ajusta automaticamente a Natureza da Operação para "Não Incidência" e tenta gravar novamente).
* **Segurança na Gravação:** O login no portal é sempre manual, com confirmação explícita do operador antes de a automação iniciar. A partir daí, o preenchimento e a gravação de cada nota (botão "Gravar Documento") acontecem automaticamente nota a nota; o operador pode pausar, retomar ou **cancelar** o robô a qualquer momento pela interface (cancelar encerra o navegador e exige reinício completo, incluindo novo login manual), e notas com campos obrigatórios ausentes ou erros de preenchimento são automaticamente puladas e registradas em log (`log_notas_incompletas.txt`), em vez de gravadas no portal.
