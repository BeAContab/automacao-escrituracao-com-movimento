...
* **Preenchimento Guiado:** Alimentação automatizada de todos os campos da escrituração (CNPJ, número da nota, data de emissão, código do serviço, descrição, valores e retenção de ISS), com verificação e log do estado final do checkbox de ISS Retido para cada nota.
...
* **Detecção de Nota Duplicada:** Quando o portal indica que já existe documento fiscal escriturado com o mesmo CNPJ do prestador e o mesmo número de nota (em outra competência), o robô automaticamente não confirma a gravação, registra a nota como ignorada em log (`log_notas_duplicadas.txt`) e segue para a próxima.
